...
function parseVariable(state: ParserState): VariableNode | null {
	...
	// Import sources frequently use human-readable field names. Collapse a run
	// of leading identifiers before expression parsing so filters still bind to
	// the full name: {{ First name | upper }}.
	if (check(state, 'identifier') && state.tokens[state.pos + 1]?.type === 'identifier') {
		const first = state.tokens[state.pos];
		let end = state.pos + 1;
		const parts = [first.value];
		while (state.tokens[end]?.type === 'identifier') {
			parts.push(state.tokens[end].value);
			end++;
		}
		state.tokens.splice(state.pos, end - state.pos, {
			...first,
			value: parts.join(' '),
		});
	}
...
export function validateFilters(
	...
	const knownFilterNames = new Set(Object.keys(registryMetadata));

	for (const usage of usages) {
		// Check if filter exists
		if (!knownFilterNames.has(usage.name)) {
			const similar = findSimilarFilter(usage.name, knownFilterNames);
