...
export const nth = (str: string, params?: string, context?: FilterContext): TemplateValue => {
	const data = arrayInputValue(str, context);
	if (!Array.isArray(data)) {
		if (typeof data === 'string' && data && data !== 'undefined' && data !== 'null') {
			try {
				JSON.parse(str);
			} catch {
	...
	if (/^\d+$/.test(nthExpression)) {
		...
		return data.filter((_, index) => index + 1 === position);
	}
