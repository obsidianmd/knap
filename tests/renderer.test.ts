...
describe('Renderer', () => {
	...
	describe('Variables', () => {
		...
		test('resolves unknown variables through a host resolver', async () => {
			const ctx = createContext();
			ctx.asyncResolver = async name => name === 'remoteValue' ? 'resolved' : undefined;

			const result = await render('{{remoteValue|upper}}', ctx);
	...
	describe('Literal values', () => {
		test('renders string literals through filters', async () => {
			const result = await render('{{"hello world"|upper}}', createContext());
			expect(result.errors).toHaveLength(0);
			expect(result.output).toBe('HELLO WORLD');
		});

		test('treats ordinary variable names without application-specific behavior', async () => {
			const result = await render('{{model}}', createContext({ model: 'local value' }));
			expect(result.output).toBe('local value');
		});
	});
