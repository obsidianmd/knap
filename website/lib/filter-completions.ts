...
import { filterDocsByName } from './filter-docs';
import type { FilterParameterSuggestion, TemplateSuggestion } from '../src/lib/playground-completions';
...
export const playgroundFilterSuggestions: TemplateSuggestion[] = Object.keys(standardFilters).map((name) => {
  const doc = filterDocsByName.get(name);
  return {
    label: name, type: 'function',
    detail: doc?.syntax[0]?.slice(doc.name.length) || undefined,
    info: doc?.summary,
    parameters: filterParameters[doc?.name ?? name],
  };
});
