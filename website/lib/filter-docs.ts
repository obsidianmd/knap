...
const recursiveMarkdownValues = 'For arrays and objects, string values are formatted recursively while keys and non-string values are preserved.';
...
const docs: FilterDoc[] = [
  ...
  { slug: 'decode-uri', name: 'decode_uri', category: 'Text', summary: 'Decode percent-encoded URI text.', syntax: ['decode_uri'], notes: ['Malformed URI sequences are returned unchanged.'], examples: [example({ value: '%E4%BD%A0%E5%A5%BD' }, '{{ value | decode_uri }}', '你好', 'Unicode text'), example({ value: 'hello%20world' }, '{{ value | decode_uri }}', 'hello world', 'Spaces')] },
  ...
  { slug: 'compact', name: 'compact', category: 'Collections', summary: 'Remove null and empty-string values from a collection.', syntax: ['compact'], notes: ['Whitespace-only strings are empty. The values `0` and `false` are preserved.', 'For objects, matching properties are removed. Nested collections are preserved.'], related: ['unique'], examples: [example({ items: [null, '', 'one', 0, false, 'two'] }, '{{ items | compact }}', '["one",0,false,"two"]', 'Array'), example({ values: { empty: '', missing: null, count: 0, enabled: false } }, '{{ values | compact }}', '{"count":0,"enabled":false}', 'Object')] },
