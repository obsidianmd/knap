...
const docs: FilterDoc[] = [
  ...
  { slug: 'callout', name: 'callout', category: 'Markdown', summary: 'Create a callout.', syntax: ['callout', 'callout:("info", "Title", false)'], parameters: ['Parameters are callout type, optional title, and optional fold state.', 'Use true for collapsed and false for expanded.'], notes: ["Output uses Obsidian's blockquote-based callout syntax. Callouts are also known as alerts or admonitions in other Markdown systems."], related: ['blockquote'], examples: [example({ message: 'Remember this' }, '{{ message | callout:("tip", "Note") }}', '> [!tip] Note\n> Remember this')] },
  {
  ...
  {
    slug: 'table', name: 'table', category: 'Markdown', summary: 'Convert arrays or objects to a Markdown table.', syntax: ['table', 'table:("Column 1", "Column 2")'],
    parameters: ['Optional parameters set column headers.'],
    notes: ['Arrays of objects use object keys as headers. Simple arrays use a single “Value” column unless custom headers are supplied.'], related: ['list'],
    examples: [
  ...
  { slug: 'merge', name: 'merge', category: 'Collections', summary: 'Append one or more values to an array.', syntax: ['merge:"value"', 'merge:("a", "b")'], related: ['unique'], examples: [example({ tags: ['notes', 'ideas'] }, '{{ tags | merge:"books" }}', '["notes","ideas","books"]', 'Append one value'), example({ tags: ['notes', 'drafts'] }, '{{ tags | merge:("ideas", "books") }}', '["notes","drafts","ideas","books"]', 'Append multiple values')] },
  ...
  { slug: 'object', name: 'object', category: 'Collections', summary: 'Convert an object to keys, values, or key-value pairs.', syntax: ['object:keys', 'object:values', 'object:array'], parameters: ['A mode parameter is required: keys, values, or array.'], examples: [example({ person: { name: 'Ada', role: 'Writer' } }, '{{ person | object:keys }}', '["name","role"]', 'Keys'), example({ person: { name: 'Ada', role: 'Writer' } }, '{{ person | object:values }}', '["Ada","Writer"]', 'Values'), example({ person: { name: 'Ada', role: 'Writer' } }, '{{ person | object:array }}', '[["name","Ada"],["role","Writer"]]', 'Key-value pairs')] },
  ...
  { slug: 'slice', name: 'slice', category: 'Collections', summary: 'Extract part of a string or array.', syntax: ['slice:1', 'slice:1,4'], parameters: ['The first index is inclusive and the second is exclusive. Negative indexes count from the end.'], related: ['first', 'last'], examples: [example({ word: 'hello' }, '{{ word | slice:1,4 }}', 'ell', 'String range'), example({ items: ['a', 'b', 'c', 'd'] }, '{{ items | slice:1,3 }}', '["b","c"]', 'Array range'), example({ word: 'hello' }, '{{ word | slice:-3 }}', 'llo', 'Negative index')] },
  ...
  { slug: 'strip-md', name: 'strip_md', category: 'HTML cleanup', summary: 'Remove Markdown formatting while keeping readable text.', syntax: ['strip_md'], related: ['strip_tags'], examples: [example({ text: '**Bold** and [linked](https://example.com)' }, '{{ text | strip_md }}', 'Bold and linked')] },
